const pokemons = [
    ...
    const container = document.getElementById('pok-con');
    render(pokemons); 
    ...
    sort.addEventListener('change', e => {
      ...
      if (sortInput === 'a-z') {
        sortedArray = [...pokemons].sort((a, b) => a.name.localeCompare(b.name));
      } else if (sortInput === 'z-a') {
        sortedArray = [...pokemons].sort((a, b) => b.name.localeCompare(a.name));
      }
    
      render(sortedArray.length ? sortedArray : pokemons);
    });
    
    const category = document.querySelector('#category');
    category.addEventListener('change', e => {
      let categoryInput = e.target.value;
      let categoryFilter = pokemons.filter(item =>
        item.type && item.type[0].toLowerCase() === categoryInput.toLowerCase()
      );
    ...
    function render(array) {
      ...
      array.forEach(item => {
        ...
        num.textContent = item.num || 'No number';
        ...
        if (item.type && item.type.length > 0) {
          ...
          const noType = document.createElement('span');
          noType.textContent = 'No Type';
          types.appendChild(noType);
        }
        card.appendChild(types);
    
        const height = document.createElement('p');
        height.textContent = `Height: ${item.height || 'Unknown'}`;
        card.appendChild(height);
    
        const weight = document.createElement('p');
        weight.textContent = ` ${item.weight || 'Unknown'}`;
        card.appendChild(weight);
    
        const candy = document.createElement('p');
        candy.textContent = ` ${item.candy || 'Unknown'}`;
        card.appendChild(candy);
    
        const spawn_time = document.createElement('h4');
        spawn_time.textContent = ` ${item.spawn_time || 'Unknown'}`;
...
category.addEventListener('change', e => {
    let categoryInput = e.target.value; // Добавлен отсутствующий символ ";" 
    let categoryFilter = pokemons.filter(
        item => item.type[0].toLowerCase() === categoryInput.toLowerCase() // Исправлен оператор "=" на "===" и добавлена стрелочная функция "=>"
    ); 
    render(categoryFilter.length ? categoryFilter : pokemons); // Исправлена скобка "}" 
});
